...
class Xml:
    ...
    -------
        ...
        _build_root_xml(xml_file_name: str) -> ET.Element:
    ...
    def __init__(self):
        ...
        self.CONFIG_XML = Config().yaml_configuration()
    ...
    def get_xml_data_list(self, xml_file_name: str) -> list:
        ...
        try:
            ...
            print(self.messages.print_message_yellow(
                message=f"* El archivo {xml_file_name} no existe en el directorio, o no es un archivo XML."))
            print(self.messages.print_message_yellow(
                message="* La estructura del archivo XML no es conocida por la aplicación."))
            print(self.messages.print_message_yellow(
                message=f"* En el archivo XML, en algún dato hace falta las etiquetas '{TAG_TO_SEARCH}' o '{XML_DOM_PATH}'"))
            print(self.messages.print_message_yellow(
                message="* En el archivo 'configXML.yaml' están mal escritas las rutas o las etiquetas."))
            print(self.messages.print_error(
    ...
    def search_the_index_in_the_list(self, data_list_csv: list, data_list_xml: list) -> list:
        ...
        try:
            ...
            print(f"uncaught exception {traceback.format_exc()}")
            print(self.messages.print_message_yellow(
                message="Verifica que el archivo CSV que no tenga datos erróneos o desconocidos."))
            print(self.messages.print_error(
                programmer_error_message="Al buscar el índice en los datos XML, puede ser que los datos en el CSV tengan un espacio, o el dato no exista en el XML",
        ...
        try:
            ...
            if file_size_in_bit > ten_million_bit:
                print(MESSAGES.print_message_yellow(
                    message=f"> El archivo → {BASENAME(path_dir=xml_file_name)} es un poco pesado, puede demorarse unos segundos en crear el nuevo archivo XML."))
    ...
    def _build_root_xml_private(self, xml_file_name: str) -> ET.Element:
        ...
        """
        try:
            ROOT_XML = self.CONFIG_XML['ROOT_XML']
            root_tag = ET.Element(ROOT_XML)
        ...
        try:
            ...
            with open(xml_file_name) as check_xml:
                ...
                if check_format is not True:
                    ...
                    print(MESSAGES.print_message_green(message=f"> El archivo → {BASENAME(path_dir=xml_file_name)} ya está formateado."))
    ...
    def get_the_text_of_a_tag(self, xml_file_name: str) -> str:
        ...
        """
        try:
            PRODUCT_PATH = self.CONFIG_XML['PRODUCT_PATH']
            root = self.get_the_root_of_the_xml_file(xml_file=xml_file_name)
            name = root.find(PRODUCT_PATH).text
            return name
        except Exception as error_when_obtaining_the_text_of_a_label:
            print(f"uncaught exception {traceback.format_exc()}")
            print(self.messages.print_message_yellow(message="Posible error es porque no existe las etiquetas en el archivo XML."))
            print(self.messages.print_error(
    ...
    def filter_data_in_xml_files(self, search: str, list_of_the_file_paths: list) -> dict:
        ...
        Args:
            ...
            list_of_the_file_paths (list): Recibe la lista con los nombres de los archivos XML y sus rutas en el sistema.
    ...
    def search_and_filter_data(self, data_dictionary: dict, data_list: list) -> tuple:
